...
> 此规则针对持续编码开发场景，优先级高于通用阶段路由。弱模型（Qwen/Doubao等）**必须按此规则执行**，不得跳过。

### 规则 C1：会话开始时——无条件执行记忆加载

只要新对话开始，不论用户说什么，**第一步必须**：

```
1. 执行"公共前置：项目上下文识别"获取 project_name / branch_name
2. 调用 init_session(project_name, branch_name)
3. 将进行中任务列表展示给用户
```
...
### 规则 C2：遇到以下编码信号时——强制检索历史

以下任一情况出现，**必须先检索再回答**，不得直接给出答案：

| 信号 | 示例 | 检索关键词 |
|------|------|----------|
| 出现报错/异常 | `AttributeError`, `ImportError`, `docker build failed` | 错误信息中的关键词 |
| 询问某个工具/库的用法 | "chromadb 怎么用" | 工具名 |
| 讨论架构/设计方案 | "这个模块怎么设计" | 模块名+design |
| 提到之前做过的功能 | "上次那个缓存优化" | 功能关键词 |
| Docker/环境/依赖问题 | "镜像构建很慢" | docker/构建/缓存 |

**执行方式**：
```
# 优先用 FTS5（速度快，适合错误信息精确匹配）
search_summaries_fts(query="报错关键词", project_name=..., limit=3)

# 若未命中，换语义检索
search_summaries(query="问题描述", use_vector=True, limit=3)
```
...
### 规则 C3：以下编码里程碑时——主动触发保存

不等用户说"保存记忆"，以下节点**自动询问**是否保存：

| 里程碑事件 | status | 说明 |
|-----------|--------|------|
| 修复了一个非显而易见的 Bug | `completed` | 含根因+解决步骤 |
| 完成一个功能模块的实现 | `completed` | 含关键设计决策 |
| 完成了一轮重构 | `completed` | 含改动理由 |
| 解决了环境/依赖/配置问题 | `completed` | 含具体操作步骤 |
| 会话即将结束但任务未完 | `in_progress` | 含 next_steps |

**执行方式**：识别到里程碑后，主动说：
> "本次完成了 [X]，是否现在保存摘要？这将帮助下次会话快速恢复上下文。"

---

### 规则 C4：同一问题重复出现时——强制提示

如果当前问题与历史检索结果高度相似（同类报错/同类配置问题），**必须在回答前说明**：
> "根据历史记录（{task_title}），这个问题之前解决过，解决方案是：[...]"

**禁止**：重新推理已有历史记录中明确解决过的问题。
